...
load("C:/Users/Lucas/Desktop/UFPR/2º Sem/Macroeoconometria/ArtigoMacroeconometria/precos.RData")

SP <- precos[grep("GUARULHOS", precos$MUNICIPIO), ]
...
aem <- ecmAsyFit(y=PREVGSP, x=PDISTGSP,lag=1, model="mtar", split=TRUE, thresh=t.mtar)
aem
(ccc <- summary(aem))
...
load("C:/Users/Lucas/Desktop/UFPR/2º Sem/Macroeoconometria/ArtigoMacroeconometria/precos1.RData")

SP <- precos1[grep("GUARULHOS", precos1$MUNICIPIO), ]
...
INPC <- as.zoo(ts(GSP$INPCFEV2016100, frequency=12,start=c(2003,1),end=c(2007,12)))
head(INPC)
...
load("C:/Users/Lucas/Desktop/UFPR/2º Sem/Macroeoconometria/ArtigoMacroeconometria/precos2.RData")

SP <- precos2[grep("GUARULHOS", precos2$MUNICIPIO), ]
